...
import './../BorrowBook/BorrowBook.css';
// import { books } from '../../_mock/data';

function ReturnBook({ setBooks, books }) {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      if (selectedBook) {
        if (selectedBook.quantity > 0) {
          console.log(`Return Book ID: ${bookId} by User ID: ${userId}`);
  ...
  return (
    <div>
      <button className='borrow-button' onClick={handleOpenModal}>Return Book</button>
      {showModal && (
        <div className="modal">
